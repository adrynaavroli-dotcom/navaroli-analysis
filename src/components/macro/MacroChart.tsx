...
export function MacroChart({ indicator, className }: MacroChartProps) {
  ...
  // For some indicators, a decrease is "good" (e.g., unemployment, stress)
  ...
  return (
    <div className={cn(
      'rounded-lg border border-slate-700/50 bg-slate-900/80 backdrop-blur-sm p-4',
      className
    )}>
      {/* Header */}
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-medium text-slate-300">{indicator.name}</h3>
        ...
        <div className="flex gap-1">
          ...
          <button
            ...
            className={cn(
              'px-2 py-0.5 text-[10px] font-mono rounded transition-colors bg-slate-800',
              scale === 'log'
                ? 'text-emerald-400'
                : 'text-slate-400 hover:text-slate-200'
            )}
