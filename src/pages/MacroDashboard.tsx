        ...
        <main className="flex-1 overflow-auto p-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={refetch}
              disabled={isLoading}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-mono text-slate-400 hover:text-slate-200 bg-slate-800 rounded-md border border-slate-700/50 transition-colors disabled:opacity-50"
            >
              {isLoading
          ...
          {categories.map((cat) => {
            ...
            return (
              <section key={cat}>
                ...
                <div className={cn(
                  'grid gap-4',
                  catIndicators.length <= 2 ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1 lg:grid-cols-2 xl:grid-cols-3'
                )}>
